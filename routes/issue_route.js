...
router.post("/issueRequest", async(req, res) => {
        ...
        const { accession_no, title, author, publisher, year, userId, bookId, userBranch, userName, userUSN, userSem, copies } = req.body;
        ...
        const avDate = await AvgDate.findOne({ accession_no: accession_no });
        if (!avDate) {
...
router.put("/issueRenewedByAdmin", async(req, res) => {
    try {
        ...
        const { accession_no, title, author, publisher, year, userId, bookId, Name, usn, Branch, semester, copies } = issue;
...
router.post("/issueReturnedAdmin", async(req, res) => {
    try {
        ...
        const avDate = await AvgDate.findOne({ accession_no: issue.accession_no });
        if (!avDate) {
        ...
        const { accession_no, title, author, publisher, year, userId, bookId, Branch, usn, Name, semester, copies } = issue;
