...
export const updateUser = (user) => async dispatch => {
    ...
    try {
        const res = await axios.post("/api/users/updateUser", user);
        console.log(res)
        dispatch({
            type: 'USER_REGISTER_SUCCESS'
...
export const filterDetails = (searchKey) => async dispatch => {
    dispatch({
        type: 'GET_STUDENTS_REQUEST'
    })
    var filterItem;
    try {
        const response = await axios.get('/api/users/allStudent');
        filterItem = response.data.filter(userData => userData.roll_no.toLowerCase().includes(searchKey.toLowerCase()));
        dispatch({
            type: 'GET_STUDENTS_SUCCESS',
            payload: filterItem
        })
...
export const getOneStudentDetails = (usn) => async dispatch => {
    dispatch({
        type: 'GET_STUDENT_REQUEST'
    })
